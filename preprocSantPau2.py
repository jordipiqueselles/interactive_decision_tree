...
def readDataSet ():
    ...
    with open('2008_2014_TANCAT_sense_V.csv', 'rt') as csvfile:
        ...
        for row in reader:
            ...
            # Diagnostics
            for familia in familiesDiag:
                auxDict[familia] = False
            diagPrinc = familiesDiag[max([i*(row["DP"][:3] >= familiesDiag[i]) for i in range(len(familiesDiag))])]
            diagSec1 = familiesDiag[max([i*(row["DS1"][:3] >= familiesDiag[i]) for i in range(len(familiesDiag))])]
            diagSec2 = familiesDiag[max([i*(row["DS2"][:3] >= familiesDiag[i]) for i in range(len(familiesDiag))])]
            auxDict[diagPrinc] = True
            auxDict[diagSec1] = True
            auxDict[diagSec2] = True
...
df = pd.DataFrame(data=readDataSet(), columns=['diesIngr', 'nIngr', 'nUrg', 'estacioAny'] + familiesDiag + ['reingres'])
df.to_csv(path_or_buf='dadesSantPauProc2.csv')
